...
class Band extends CI_Controller {
	...
	public function add(){
		...
		if ( ! empty($band_id) && ! empty($name) && $is_master == 1) {
			...
			if ($is_uploaded == 0) {
				...
				$this->band_album_model->add($data);
	...
	public function delete($album_id) {
		...
		if ($album->band_id == $band_id && $is_master == 1) {
			$this->band_album_model->delete($album_id);
			$username = $this->session->userdata('username');

			redirect('band/' . $band_id . '/music');
		} else {
			show_404();
		}
	}
	...
	public function view($album_id) {
		...
		if ( ! empty($album)) {
			...
			// Page data
			$albums = $this->band_album_model->get_by_band($band_profile->id);
