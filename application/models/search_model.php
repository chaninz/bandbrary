...
class Search_model extends CI_Model {
	...
	function music($words){
		$query = $this->db->query('
			SELECT User_Music.id as musicid,User_Music.name as musicname
			FROM User_Music
			...
			UNION ALL
			SELECT Band_Music.id as musicid,Band_Music.name as musicname
			FROM Band_Music
	...
	function album($words){
		$query = $this->db->query('
			SELECT User_Albums.id as albumid,User_Albums.name as albumname
			FROM User_Albums
			JOIN Users ON Users.id = User_Albums.user_id
			...
			SELECT Band_Albums.id as albumid,Band_Albums.name as albumname 
			FROM Band_Albums 
			JOIN Bands ON Bands.id = Band_Albums.band_id
